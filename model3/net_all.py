...
class NetA(object):
    ...
    def create_upscale_concat_conv_bn_layer(self, x, y, h_size, layer_i):
            ...
            x1 = tf.layers.conv2d_transpose(x, h_size, kernel_size=[3,3], strides=[2, 2], padding='same', activation=tf.nn.elu, reuse=False, name="deconv_"+str(layer_i))
            x1 = tf.image.resize_images(x1, y.get_shape()[1:3])
            x2 = tf.image.resize_bilinear(x, y.get_shape()[1:3])
            x10 = tf.concat([x1, x2, y], axis=-1)

            branch_list = list()
            branch_list.append(x10)
            i=0

            x11 = tf.layers.conv2d(x10, h_size, kernel_size=[1, 1], strides=[1, 1], padding='same', activation=tf.nn.elu, reuse=False, name="conv_"+str(layer_i)+'_'+str(i))
            x11 = self.group_norm(x11, scope=str(layer_i)+'_group_norm'+'_'+str(i)) 
            i += 1
            branch_list.append(x11)
            
            x12 = tf.layers.conv2d(x11, h_size//2, kernel_size=[3, 3], strides=[1, 1], padding='same', activation=tf.nn.elu, reuse=False, name="conv_"+str(layer_i)+'_'+str(i))
            x12 = self.group_norm(x12, scope=str(layer_i)+'_group_norm'+'_'+str(i)) 
            i += 1
            branch_list.append(x12)

            x20 = tf.concat(branch_list, axis=-1)   
            x20 = tf.layers.conv2d(x20, h_size, kernel_size=[1, 1], strides=[1, 1], padding='same', activation=tf.nn.elu, reuse=False, name="conv1x1_"+str(layer_i)+'_'+str(i))
    ...
    def dense_block(self, x, h_size, layer_i):  
            ...
            i=0
            x_max = tf.layers.max_pooling2d(x, pool_size=[3,3], strides=[1,1], padding='same', data_format='channels_last', name="maxpool_"+str(layer_i)+'_'+str(i))
            x0 = tf.concat([x, x_max], axis=-1)
            branch_list = list()
            branch_list.append(x0)
            
            x1 = tf.layers.conv2d(x0, h_size, kernel_size=[1, 1], strides=[1, 1], padding='same', activation=tf.nn.elu, reuse=False, name="conv_"+str(layer_i)+'_'+str(i))
