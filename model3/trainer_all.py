...
class Trainer(object):
  ...
  def _load_graph(self):
        with self.graph.as_default():
            ...
            if ckpt is None:
                logger.info('The model {0} could not be found. Make '.format(self.load_model_path))
  ...
  def save_model(self, step):
        with self.graph.as_default():
            ...
            tf.train.write_graph(self.graph, self.model_path, 'raw_graph_def.pb', as_text=False)

  def update(self, inputs, true_label, input_times):
  ...
  def infer(self, inputs, input_times, ):
        feed_dict = {
                     self.model.batch_size: len(inputs),
                     self.model.visual_in: inputs,
                     self.model.vector_in: input_times,
  ...
  def evaluate(self, inputs, true_label, input_times):
        feed_dict = {
                     self.model.batch_size: len(inputs),
                     self.model.visual_in: inputs,
                     self.model.vector_in: input_times,
